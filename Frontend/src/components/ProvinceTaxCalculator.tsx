...
interface TaxCalculatorProps {
  ...
  updateProvinceTaxAmount: (taxAmount: number) => void; // Add updateTaxAmount prop
}

const ProvinceTaxCalculator: React.FC<TaxCalculatorProps> = ({
  selectedProvince,
  principalAmount,
  updateProvinceTaxAmount,
}) => {
  const [_provinceTaxAmount, setProvinceTaxAmount] = useState<number>(0);
    ...
    if (selectedProvince) {
      axios
        ...
        .then((response) => {
          ...
          setProvinceTaxAmount(tax);
          updateProvinceTaxAmount(tax);
        })
        .catch((error) => {
          console.error("Error fetching tax data:", error);
        });
    }
  }, [selectedProvince, principalAmount, updateProvinceTaxAmount]);

  return null;
};

export default ProvinceTaxCalculator;
